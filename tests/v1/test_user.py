...
class TestUser(unittest.TestCase):
    ...
    def test_signup_user_shortusername(self):
        res = self.client.post('/api/v1/signup', data=json.dumps({'user_name' : "ma", 'password' : "markmain", "email" : 'mbuguamark@gmail.com'}), content_type='application/json')
        result = json.loads(res.data)
    ...
    def test_signup_user_sparcele_password(self):
        res = self.client.post('/api/v1/signup', data=json.dumps({'user_name' : "marky", 'password' : "markma ", "email" : 'mbuguamark@gmail.com'}), content_type='application/json')
        result = json.loads(res.data)
        self.assertEqual(result["Message"], "Password should be one word, no spaces")
        self.assertEqual(res.status_code, 400)
    
    def test_signup_user_short_password(self):
        res = self.client.post('/api/v1/signup', data=json.dumps({'user_name' : "marky", 'password' : "mark", "email" : 'mbuguamark@gmail.com'}), content_type='application/json')
        result = json.loads(res.data)
